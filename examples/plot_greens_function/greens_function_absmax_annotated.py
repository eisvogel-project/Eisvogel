...
def greens_function_absmax_annotated(exported_green_path, outpath, config_path, fs = 15):
    ...
    fieldplot = plotting_utils.plot_field(ax_field, E_abs_max_data, range_x = config["range_x"], range_y = config["range_y"], epilog = field_annotation_epilog)
    ...
    for ind, (cur_annotation, cur_title) in enumerate(zip(annotations, annotation_labels)):
        ...
        ax_annotation.plot(tvals, E_z_data[int(cur_detail_pos_ind[0]), int(cur_detail_pos_ind[1]), :], color = "black", label = r"$E_z$")
        ax_annotation.plot(tvals, E_r_data[int(cur_detail_pos_ind[0]), int(cur_detail_pos_ind[1]), :], color = "gray", label = r"$E_r$")
