namespace stor {
  ...
  void DenseNDVecArrayStreamer<T, dims, vec_dims>::serialize(std::fstream& stream, const type& val) {
  ...
  template <typename T, std::size_t dims, std::size_t vec_dims>
  DenseNDVecArrayStreamer<T, dims, vec_dims>::type DenseNDVecArrayStreamer<T, dims, vec_dims>::deserialize(std::fstream& stream) {
