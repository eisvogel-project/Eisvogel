...
void create_greens_function(std::filesystem::path outdir, scalar_t filter_t_peak, unsigned int filter_order) {
  ...
  RZCoordVector end_coords{500.0f, 100.0f};
  scalar_t t_end = 500;
  scalar_t ior = 1.0;
...
void calculate_signal_eisvogel(std::filesystem::path gf_path, scalar_t b, scalar_t beta,
  ...
  for(std::size_t i = 0; i < 2; i++) {
    ...
    for(scalar_t cur_t = tstart; cur_t < tend; cur_t += delta_t_traj) {
      LineCurrentSegment track(XYZCoordVector{beta * cur_t, 0.0f, b},                    // track start position
			       XYZCoordVector{beta * (cur_t + delta_t_traj), 0.0f, b},   // track end position
			       cur_t, cur_t + delta_t_traj, charge);
...
void run_test(std::filesystem::path result_path, std::filesystem::path gf_path, scalar_t b, scalar_t beta, float rel_th = 8e-5) {
  ...
  std::vector<scalar_t> delta_ts_traj{10, 1.0, 0.5, 0.1};
...
int main(int argc, char* argv[]) {
...

  {
    scalar_t b = 5;
    scalar_t beta = 0.9;
    std::filesystem::path result_path = validation_dir / "b_5_beta_0.9_tpeak_5.0_order_4.csv";
