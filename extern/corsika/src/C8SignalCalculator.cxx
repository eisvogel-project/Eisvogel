...
void C8SignalCalculator::calculate(const std::array<float, 4>& track_start_xyzt, const std::array<float, 4>& track_end_xyzt, float track_charge,
				   float t_sig_start, float t_sig_samp, std::size_t num_samples, std::vector<float>& signal, float weight) {
  ...
  m_gf -> apply_accumulate<Interpolation::Kernel::Linear>(track, t_sig_start, t_sig_samp, num_samples, signal, oob_mode, weight);
}
