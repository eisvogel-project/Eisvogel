...
struct ChunkMetadata {
  ...
  // (Can be empty if `ChunkType == all_null`)
...
public:
  ...
  metadata_t& RegisterChunk(const Vector<std::size_t, dims>& start_ind, const Vector<std::size_t, dims>& shape, std::size_t overlap,
			    const ChunkType& chunk_type = ChunkType::specified);
...
public:
  ...
  void RegisterChunk(const chunk_t& chunk, const ind_t& start_ind, const ChunkHints& hints = ChunkHints::NONE);
  void RegisterChunk(const chunk_t& chunk, const ind_t& start_ind, const ind_t& end_ind, std::size_t overlap,
		     const ChunkHints& hints = ChunkHints::NONE);
